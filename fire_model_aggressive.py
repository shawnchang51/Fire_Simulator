"""
Aggressive Indoor Fire Spread Model using Cellular Automata
============================================================

High-intensity fire simulation model for stress-testing evacuation algorithms.
Designed for worst-case scenarios and algorithm validation.

SPATIAL/TEMPORAL ASSUMPTIONS:
- Cell size: 0.3m × 0.3m (representing fine-grained indoor space)
- Timestep: 0.5 seconds
- Fire update interval: Every 2-3 timesteps (1-1.5 seconds)

FIRE INTENSITY SCALE (Physical Interpretation):
- 0: Clear/Unburned (20°C ambient)
- 1: Ignition/Smoldering (100-300°C, <100 kW/m²)
- 2: Growth phase (300-500°C, 100-500 kW/m²)
- 3: Fully developed (500-800°C, 500-1000 kW/m²)
- 4: Flashover (>800°C, >1000 kW/m²)
- -1: Inaccessible areas (no oxygen, extreme conditions)
- -2: Permanent obstacles (walls, barriers)

AGGRESSIVE FIRE BEHAVIOR (Testing Scenarios):
- Time to flashover: 30-60 seconds
- Spread speed: 0.3-0.5 m/s (high wind, optimal fuel conditions)
- Adjacent cell ignition: 40-60% probability
- Burn duration: 40+ seconds before decay
- Room engulfment: 2-3 minutes for 10×10m space

Use this model for: Stress-testing evacuation algorithms, worst-case scenario
planning, algorithm performance validation, emergency response training.

WARNING: This model is intentionally aggressive and represents worst-case
conditions. Real fires typically spread slower. Use realistic model for
accurate evacuation planning.
...
class EnvironmentalParameters:
    """Environmental conditions affecting fire spread - AGGRESSIVE SETTINGS"""
    # Wind and airflow - HIGH WIND CONDITIONS
    wind_speed: float = 1.5  # m/s (increased from 0.5 for aggressive spread)
    ...
    # Advanced physics parameters - AGGRESSIVE SETTINGS
    thermal_conductivity: float = 0.5  # heat transfer coefficient
    ignition_threshold: float = 0.2  # probability threshold for ignition (EASIER TO IGNITE)
    burn_rate_modifier: float = 1.5  # overall fire spread speed (FASTER SPREAD)
...
class AdvancedFireModel:
    ...
    AGGRESSIVE CONFIGURATION - For stress-testing and worst-case scenarios
    """
    ...
    def _calculate_spread_probability(self, current_state: List[List[float]],
        ...
        AGGRESSIVE VERSION - Higher spread probabilities
        """
        ...
        # Base spread probability - AGGRESSIVE: FASTER SPREAD
        base_prob = min(neighbor_fire_sum * 0.08, 0.7)  # Increased from realistic 0.03 and 0.5
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if 0 < fire_intensity <= 4:
                    # Fire consumes oxygen - AGGRESSIVE: MODERATE CONSUMPTION
                    oxygen_consumption = fire_intensity * 0.15  # Faster than realistic (0.05)
                    self.oxygen_map[i][j] = max(0, self.oxygen_map[i][j] - oxygen_consumption)

                    # Fire increases temperature - AGGRESSIVE: FASTER HEATING
                    heat_production = fire_intensity * 12.0  # Increased from realistic 5.0
    ...
    def _calculate_fire_progression(self, current_intensity: float, row: int, col: int) -> float:
        ...
        if current_intensity < 4.0:
            ...
            if oxygen_factor > 0.7 and fuel_factor > 0.1:
                # AGGRESSIVE: FAST GROWTH (30-60 seconds to flashover)
                growth_rate = 0.2 * oxygen_factor * fuel_factor  # Increased from realistic 0.08
        ...
        if self.fuel_map[row][col] < 0.1:
            decay_rate = 0.15  # Slower decay - fire persists
        # Oxygen starvation causes decay
        elif self.oxygen_map[row][col] < 10.0:
            decay_rate = 0.15  # Slower decay
        # Natural burnout after extended burning - AGGRESSIVE: LONGER BURN
        elif burn_duration > 40.0:  # 40 steps at 1s updates = 40 seconds
            decay_rate = 0.15  # Slower decay
        else:
    ...
    def get_simulation_statistics(self) -> Dict[str, float]:
        """Return advanced statistics for fire analysis"""
...
def create_fire_model(rows: int, cols: int, **env_kwargs) -> AdvancedFireModel:
    ...
    Example usage:
        model = create_fire_model(20, 30, wind_speed=2.0, humidity=20.0)
...
if __name__ == "__main__":
    ...
    if sys.stdout.encoding and 'utf' in sys.stdout.encoding.lower():
        print("🔥 AGGRESSIVE Indoor Fire Simulation Model 🔥")
    else:
        print("*** AGGRESSIVE Indoor Fire Simulation Model ***")
    print("=" * 50)
    print("High-intensity parameters for stress-testing evacuation algorithms")
    print("Time to flashover: 30-60 seconds | Spread: 0.3-0.5 m/s")
    print("WARNING: Worst-case scenario - not typical fire behavior")
    print("=" * 50)
    ...
    # Create model with aggressive conditions
    ...
    print(f"\nInitial fire state: Fire started at position (2,2)")
    ...
    print(f"  Burn rate modifier: {model.env.burn_rate_modifier} (aggressive)")
    ...
    for step in range(5):
        ...
        if changes:
            ...
            for pos, new_val in list(changes.items())[:5]:  # Show first 5 changes
    ...
    print("\nAggressive Fire Model Features:")
    print("• Rapid fire spread (0.3-0.5 m/s)")
    print("• Fast flashover (30-60 seconds)")
    print("• High wind conditions (1.5 m/s)")
    print("• Extended burn duration")
    print("• Ideal for stress-testing evacuation algorithms")
    print("\n⚠️  Use realistic model for actual evacuation planning")
