"""
Realistic Indoor Fire Spread Model using Cellular Automata
===========================================================

Physics-aligned fire simulation model for realistic indoor fire behavior.
Designed for accurate evacuation planning and fire safety analysis.

SPATIAL/TEMPORAL ASSUMPTIONS:
- Cell size: 0.3m × 0.3m (representing fine-grained indoor space)
- Timestep: 0.5 seconds
- Fire update interval: Every 4-5 timesteps (2-2.5 seconds)
...
- Time to flashover: 3-6 minutes
- Spread speed: 0.1-0.2 m/s (typical indoor conditions)
- Adjacent cell ignition: 15-30% probability
- Burn duration: 2+ minutes before decay

Use this model for: Accurate evacuation simulations, fire safety analysis,
building code compliance, realistic training scenarios.
...
class EnvironmentalParameters:
    ...
    ignition_threshold: float = 0.5  # probability threshold for ignition (HARDER TO IGNITE)
    burn_rate_modifier: float = 0.3  # overall fire spread speed (SLOWER SPREAD)
...
class AdvancedFireModel:
    ...
    def _calculate_spread_probability(self, current_state: List[List[float]],
        ...
        # Base spread probability - REALISTIC: SLOWER SPREAD
        base_prob = min(neighbor_fire_sum * 0.03, 0.5)  # Reduced from 0.1 and 0.8
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if 0 < fire_intensity <= 4:
                    # Fire consumes oxygen - REALISTIC: SLOWER CONSUMPTION
                    oxygen_consumption = fire_intensity * 0.05  # Reduced from 0.5
                    self.oxygen_map[i][j] = max(0, self.oxygen_map[i][j] - oxygen_consumption)

                    # Fire increases temperature - REALISTIC: SLOWER HEATING
                    heat_production = fire_intensity * 5.0  # Reduced from 50.0
    ...
    def _calculate_fire_progression(self, current_intensity: float, row: int, col: int) -> float:
        ...
        if current_intensity < 4.0:
            ...
            if oxygen_factor > 0.7 and fuel_factor > 0.1:
                ...
                growth_rate = 0.08 * oxygen_factor * fuel_factor  # Reduced from 0.3
        ...
        if self.fuel_map[row][col] < 0.1:
            decay_rate = 0.3  # Reduced from 0.4
        # Oxygen starvation causes decay
        elif self.oxygen_map[row][col] < 10.0:
            decay_rate = 0.3
        # Natural burnout after extended burning - REALISTIC: LONGER BURN
        elif burn_duration > 60.0:  # Increased from 20.0 (now 2 minutes at 2s updates)
            decay_rate = 0.2
        else:
...
if __name__ == "__main__":
    ...
    if sys.stdout.encoding and 'utf' in sys.stdout.encoding.lower():
        print("🔥 Realistic Indoor Fire Simulation Model 🔥")
    else:
        print("*** Realistic Indoor Fire Simulation Model ***")
    print("=" * 50)
    print("Physics-aligned parameters for accurate evacuation planning")
    print("Time to flashover: 3-6 minutes | Spread: 0.1-0.2 m/s")
    print("=" * 50)
    ...
    print(f"  Burn rate modifier: {model.env.burn_rate_modifier} (realistic)")
    ...
    for step in range(5):
        ...
        if changes:
            ...
            for pos, new_val in list(changes.items())[:3]:  # Show first 3 changes
    ...
    print("\nRealistic Fire Model Features:")
    print("• Physics-aligned fire spread (0.1-0.2 m/s)")
    print("• Realistic time to flashover (3-6 minutes)")
    print("• Accurate oxygen consumption rates")
    print("• Temperature rise aligned with real fires")
    print("• Suitable for evacuation planning and building codes")
