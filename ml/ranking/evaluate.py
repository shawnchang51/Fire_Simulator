"""
Evaluation Metrics for Pairwise Ranking Model
...
def evaluate_per_plan_ranking(
    ...
    # OPTIMIZATION: Batch score all configs at once, then group by plan
    all_grids = []
    all_scenarios = []
    all_plan_ids = []
    all_true_scores = []
...
def evaluate_model_full(
    ...
    """
    Full evaluation: pairwise + per-plan ranking metrics.

    Args:
        model: Trained ranking model
        test_loader: Test data loader for pairwise metrics
        eval_dataset: SingleConfigDataset for per-plan metrics
        device: Device to evaluate on
